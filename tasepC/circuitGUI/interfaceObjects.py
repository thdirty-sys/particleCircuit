...
class TasepCircuitSimulateGUI():
    ...
    def start(self):
        ...
        dpg.add_group(tag="sim_control", parent="control")
        dpg.add_progress_bar(label="Computing...", default_value=0.0, tag="sim_progress_bar", parent="sim_control")
        dpg.add_text(tag="progress_bar_text", default_value="Computing... 0%", parent="sim_control")
        dpg.add_button(tag="cancel_sim_button", width=200, height=30, label="Cancel", callback=self.cancel,
                       parent="sim_control")
    ...
    def run_tasep(self):
        ...
        time = 10000
        relevant_fraction = round(time*0.01)

        # Shorthand
        c = self.circuit
        while self.particle_count < time and not self.cancelled:
                ...
                if self.pos_empty(chosen.pos, c.particles):
                    ...
                    if rng.random() <= chosen.rate:
                        if self.pos_empty(chosen.pos, c.path_orientation[chosen.pos]):
                            ...
                            if self.particle_count % relevant_fraction == 0:
                                ratio = round(self.particle_count / time, 2)
                                dpg.configure_item("sim_progress_bar", default_value=ratio)
                                dpg.configure_item("progress_bar_text", default_value=f"Computing... {100 * ratio}%")
        ...
        dpg.delete_item("sim_progress_bar")
        dpg.delete_item("progress_bar_text")

        if not self.cancelled:
            dpg.delete_item("cancel_sim_button")
            for node in self.tracked:
                count = floor(node.check_in[0])
                self.data.currents_1[node.pos] = [0 for i in range(count+1)]
            ...
            graphs = StatisticalFrames(self.data)
            graphs.setup_frames()
            dpg.add_button(width=200, height=30, label="Save data", callback=self.save_data,
                           parent="sim_control")
            dpg.add_button(width=200, height=30, label="Back", callback=self.exit_sim,
                           parent="sim_control")
...
class TasepCircuitDispatcherGUI(TasepCircuitSimulateGUI):
        ...
        if self.run:
            for node in self.tracked:
                if node.check_in:
                    ...
                    count = floor(node.check_in[0])
                    top = floor(node.check_in[-1])
                    if not self.data.currents_1[node.pos]:
                        self.data.currents_1[node.pos] = [0 for i in range(count+1)]
                    if top != count:
